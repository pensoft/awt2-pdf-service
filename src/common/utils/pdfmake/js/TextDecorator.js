  ...
  for (let i = 0, l = line.inlines.length; i < l; i++) {
    ...
    for (let ii = 0, ll = decoration.length; ii < ll; ii++) {
      ...
      if (
				...
				sub !== currentGroup.decorationSub||
				sup !== currentGroup.decorationSup||
				fontSize !== currentGroup.decorationFontSize
				) {
        currentGroup = {
          line: line,
...
class TextDecorator {
  ...
  _drawDecoration(group, x, y) {
		/* group.decorationColor = '#'+Math.floor(Math.random()*16777215).toString(16); */
    const maxInline = () => {
      let max = 0;
    ...
    switch (group.decoration) {
      ...
      default:
		...
		/* decorationSub:sub,
					decorationSup:sup,
					decorationFontSize:fontSize */
		if(group.decorationSub){
			y += biggerInline.fontSize*0.4;
		}
		if(group.decorationSup){
			y -= biggerInline.fontSize*0.7;
		}
