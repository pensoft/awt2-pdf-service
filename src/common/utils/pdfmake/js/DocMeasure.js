...
class DocMeasure {
  ...
  measureDocument(docStructure) {
		this.svgs = {};
		return {docStructure:this.measureNode(docStructure),svgs:this.svgs};
  }
    ...
    return this.styleStack.auto(node, () => {
      ...
      if (node.columns) {
        ...
        return extendMargins(this.measureToc(node));
				...
				if(node.imgId&&node.inlineimg){
					this.svgs[node.imgId] = node;
				}
        return extendMargins(this.measureImage(node));
				...
				if(node.svgId&&node.inlinesvg){
					this.svgs[node.svgId] = node;
				}
