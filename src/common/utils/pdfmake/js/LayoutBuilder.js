...
let { calcH,ordNodesFunc } = require('./reorderNodes');
...
class LayoutBuilder {
  ...
  layoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark, pageBreakBeforeFct,pdfSettings) {
    ...
    let result = this.tryLayoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark,pdfSettings,false);

		result = calcH(result.linearNodeList, result.pages,docStructure,this.writer,pdfDocument);
		resetXYs(result);
    result = this.tryLayoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark,pdfSettings, true);
...
/* 		resetXYs(result);
    result = this.tryLayoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark,pdfSettings, false); */
...
/*     while (addPageBreaksIfNecessary(result.linearNodeList, result.pages)) {
  ...
  tryLayoutDocument(docStructure, pdfDocument, styleDictionary, defaultStyle, background, header, footer, watermark,pdfSettings, shouldreorder) {
    this.linearNodeList = [];
    docStructure = this.docPreprocessor.preprocessDocument(docStructure);

    let data = this.docMeasure.measureDocument(docStructure);
		docStructure = data.docStructure;
		this.svgs = data.svgs;
    this.writer = new _PageElementWriter.default(new _DocumentContext.default(this.pageSize, this.pageMargins),this.pdfSettings);
    ...
    this.processNode(docStructure,pdfSettings,shouldreorder);
  ...
  addBackground(background) {
    ...
    if (pageBackground) {
      ...
      this.processNode(this.docMeasure.measureDocument(pageBackground).docStructure);
  ...
  addDynamicRepeatable(nodeGetter, sizeFunction) {
    ...
    for (let pageIndex = 0, l = pages.length; pageIndex < l; pageIndex++) {
      ...
      if (node) {
        ...
        this.processNode(this.docMeasure.measureDocument(node).docStructure);
  ...
  processNode(node,pdfSettings, shouldreorder) {
    ...
    decorateNode(node);
		...
		let availableHeightBefore = this.writer.context().availableHeight;
		let pageBefore = this.writer.context().page;
		let yBefore = this.writer.context().y;
		let xBefore = this.writer.context().x;
    ...
    applyMargins(() => {
      ...
      if (node.stack) {
        this.processVerticalContainer(node,pdfSettings,shouldreorder);
      ...
      if (unbreakable) {
		...
		let availableHeightAfter = this.writer.context().availableHeight;
		let pageAfter = this.writer.context().page;
		let yAfter = this.writer.context().y;
		let xAfter = this.writer.context().x;
			node.nodePosMeta = {
				availableHeightBefore,
				availableHeightAfter,
				pageBefore,
				pageAfter,
				yBefore,
				yAfter,
  ...
  processVerticalContainer(node,pdfSettings,shouldreorder) {
		for(let i = 0 ; i < node.stack.length ; i++){
			...
			getData();
			if(item.props&&item.props.main&&shouldreorder){
				if(ordNodesFunc(item,node.stack,pdfSettings,this.writer,i)){
					getData();
				}
			}
			this.processNode(item,shouldreorder);
    ...
    const addMarkerToFirstLeaf = line => {
      ...
      if (nextMarker) {
        ...
        if (marker.canvas) {
          ...
          let markerLine = new _Line.default(this.pageSize.width,this.svgs,this.pdfSettings);
  ...
  buildNextLine(textNode) {
    ...
    let line = new _Line.default(this.writer.context().availableWidth,this.svgs,this.pdfSettings);
